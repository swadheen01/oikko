...
- ✅ **Profile** — view + edit own profile (name, school with autocomplete, designation, phone, blood group, **index number, joining date, MPO date**, photo upload). Sign-up and the Google setup screen collect the same fields; all show on the member's profile.
...
- ✅ **About / committee** — About page (association info, office bearers with photos) + full **committee list** (55 members, imported from the 2026 Excel via Bijoy→Unicode) that admins can edit inline (name, designation, photo) — backed by the `committee` collection (`c0`..`c54`, defaults overridable).
- ✅ **Landing website** — static bilingual site in `docs/` for **GitHub Pages** (https://swadheen01.github.io/oikko/): association info, features, office bearers + developer section, and a Download button that always fetches the newest APK from the repo's **latest GitHub Release** (asset must stay named `app-release.apk`). Publishing a new APK = new Release, no site edit.
...
**Operational note — removing a member:** admin "Delete member" removes the member's Firestore data (member doc + transactions) only. It **cannot** delete the person's Firebase **Auth** account (client SDK can't delete another user; needs the Admin SDK on a server). To free the email for re-registration, delete that user manually in **Firebase Console → Authentication**.
